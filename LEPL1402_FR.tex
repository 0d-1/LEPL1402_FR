...
Vous trouverez plus d'informations sur Unicode sur
...
La documentation complète de la classe \texttt{Math} est disponible à l'adresse
...
\textbf{Code Java} & \textbf{En mémoire pendant l'exécution} \\
...
\emph{Conseil :} évitez autant que possible le masquage, car il peut facilement
entraîner des erreurs de compréhension.
...
Si vous oubliez les accolades, seule la première instruction appartient
vraiment à la boucle, quel que soit l'indentation :

\begin{lstlisting}[language=Java]
int sum = 0;
int i = 0;
while (i < 10)
    // oops, on a oublie les accolades
    sum += i; // cette instruction est DANS la boucle
    System.out.println("Nearly there"); // cette instruction est HORS boucle
    i++; // cette instruction est HORS boucle
System.out.println("The sum is " + sum);
\end{lstlisting}

Cela vaut aussi pour les autres boucles et pour les instructions
\texttt{if/else}.
...
La boucle \texttt{for} s'exécute autant de fois qu'il y a d'éléments dans le
tableau, et la variable \texttt{elem} prend successivement la valeur de chaque
élément.
...
La première ligne du \texttt{for} est composée de trois éléments :

\begin{enumerate}
  \item une instruction exécutée au démarrage (ici : \texttt{int i = 0}) ;
  \item une expression évaluée \emph{avant} chaque itération ; si elle est
        fausse, la boucle s'arrête (ici : \texttt{i < 10}) ;
  \item une instruction exécutée \emph{après} chaque itération
        (ici : \texttt{i++}).
\end{enumerate}

La forme complète est plus flexible car elle vous donne un contrôle fin sur le
...
Dans cet exemple, on utilise \texttt{myArray.length} pour connaître la taille
du tableau, et l'instruction \texttt{i += 2} pour avancer de deux positions
à chaque itération.
...
\texttt{break} permet de sortir de la boucle :
...
\texttt{continue} permet de passer directement à l'itération suivante :
...
À n'utiliser que s'ils rendent le code plus lisible. Par exemple, ce code :
...
Comme pour les boucles, n'oubliez pas les accolades \texttt{\{...\}} quand le
corps de l'instruction contient plusieurs lignes. Il est recommandé de les
...
\noindent L'instruction \texttt{if} attend une expression booléenne, c'est-à-dire
une expression qui vaut \texttt{true} ou \texttt{false}. Les opérateurs
booléens sont très proches de ceux de Python :
...
for (char c : directions) {
    if (c == 'N') {
        y++; // on va vers le Nord
    } else if (c == 'S') {
        y--; // on va vers le Sud
    } else if (c == 'W') {
        x--; // on va vers l'Ouest
    } else if (c == 'E') {
        x++; // on va vers l'Est
    } else {
...
Java propose une instruction \texttt{switch} qui rend ce code plus compact :
...
switch (c) {
    case 'N':
        y++;
        break; // sans break, les effets sont inattendus
    case 'S':
        y--;
        break;
    case 'W':
...
Comme Java~8 est encore largement utilisé, il est utile de connaître les deux
syntaxes.
...
La classe \texttt{String} propose de nombreuses méthodes utiles. Si vous
...
vous verrez que certaines méthodes sont \emph{statiques} et d'autres non. Par
exemple, la méthode statique \texttt{valueOf} transforme une valeur numérique
en chaîne :
...
La plupart des méthodes de \texttt{String} ne sont pas statiques : elles
s'appellent sur une chaîne ou une variable de type \texttt{String}. Voici des
méthodes courantes :
...
Des méthodes similaires existent dans \texttt{Long}, \texttt{Float},
\texttt{Double}, etc. Toutes ces classes sont définies dans le package
\texttt{java.lang}, dont la documentation est disponible à l'adresse
\url{https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/package-summary.html}.

Vous pourriez vous demander pourquoi on n'importe pas explicitement
\texttt{Math}, \texttt{Integer} ou \texttt{String} dans nos exemples : c'est
parce que \texttt{java.lang} est le seul package importé automatiquement par le
compilateur Java.
...
Pour les types primitifs, \texttt{==} compare les valeurs :
...
Pour les tableaux et les chaînes, \texttt{==} compare les références, pas le
contenu :
...
Comparer des tableaux ou des chaînes avec \texttt{==} est une erreur fréquente :
rappelez-vous que cela compare les références, pas le contenu.
...
Pour les tableaux, on utilise \texttt{Arrays.equals}, une méthode statique de
\texttt{java.util.Arrays}. Voici le code complet :
...
La classe \texttt{Arrays} contient de nombreuses méthodes utiles pour les
tableaux, par exemple pour initialiser tous les éléments, copier un tableau ou
convertir un tableau en chaîne. Consultez la documentation :
\url{https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/Arrays.html}.

Vous pourriez vous demander pourquoi on doit écrire
\texttt{import java.util.Arrays} alors qu'on n'a pas eu besoin d'importer
\texttt{Math}, \texttt{Integer} ou \texttt{String} : c'est parce que ces
classes appartiennent à \texttt{java.lang}, le seul package importé
automatiquement.
...
Les \texttt{enum} définissent un ensemble de constantes typées (des valeurs
immuables, similaires à des variables \texttt{final}). Elles améliorent la
lisibilité et permettent de regrouper des valeurs nommées dans un espace de
noms dédié.

Il est pratique d'utiliser \texttt{switch} pour réagir à une valeur d'énumération.
Comme il n'existe qu'une seule instance de chaque constante dans la JVM, vous
pouvez aussi comparer les valeurs avec \texttt{==}.

Reprenons l'exemple des directions, mais avec une \texttt{enum} plutôt que des
\texttt{char}. Le code devient plus sûr, car il est impossible d'utiliser une
direction qui n'appartient pas à la liste.
...
public class DirectionFollower {
    ...
    /**
     * Calcule les coordonnees finales apres une serie de mouvements a partir
     * d'une position initiale.
     *
     * @param start Tableau de taille deux : start[0] est x, start[1] est y.
     * @param directions Tableau de valeurs de l'enum Direction.
     * @return Nouveau tableau de taille deux avec les coordonnees finales.
     *
     * Exemple :
     * {@code
     * int[] start = {0, 0};
...
Dans cet exemple, \texttt{setBossOfTeam()} ne capture pas les exceptions
éventuelles de \texttt{setBoss()}, donc l'exception est propagée à
\texttt{main()}, où elle est enfin interceptée.
...
Cela fonctionne parce que \texttt{catch(XYZ e)} intercepte toutes les
exceptions de la classe \texttt{XYZ} et de ses sous-classes, tant qu'aucun
autre \texttt{catch} ne traite un sous-type plus spécifique.
...
dérivent de \texttt{Throwable}. Toute instance de \texttt{Throwable} (ou de
ses sous-classes) peut être levée avec l'instruction \texttt{throw}.

La hiérarchie de ces classes est la suivante :
...
Comparez par exemple ces deux lignes :
...
Il n'est pas évident, à la lecture, que la seconde ne sera pas exécutée si la
première échoue. La version sans exceptions le rend explicite :
...
Dans la pratique, on n'a pas souvent besoin de lever ses propres exceptions ;
en revanche, on doit fréquemment en intercepter lors d'opérations d'entrée/
sortie, comme montré dans la section suivante.
...
Le constructeur de \texttt{FileReader} peut lever
\texttt{FileNotFoundException} si le fichier n'existe pas. La méthode
\texttt{read()} peut lever \texttt{IOException} en cas d'erreur de lecture,
par exemple si l'utilisateur n'a pas les droits. Comme
\texttt{FileNotFoundException} est une sous-classe de \texttt{IOException}, un
seul \texttt{catch (IOException e)} suffit.
...
La JVM exécute \emph{toujours} le bloc \texttt{finally} après le bloc
\texttt{try}, même si une exception est levée ou si le \texttt{try} contient un
\texttt{return}. C'est pourquoi \texttt{finally} est souvent utilisé pour
libérer des ressources (comme fermer un fichier).
...
Cette forme compacte est équivalente à la version précédente : le compilateur
ajoute automatiquement un \texttt{finally} et l'appel à \texttt{close()}.
...
Les programmes informatiques consistent à organiser des données et à travailler
avec ces données. Dans certaines applications, les types primitifs, les
tableaux et les chaînes suffisent, mais on rencontre souvent des données plus
complexes. Par exemple, imaginons un programme qui gère des employés d'une
entreprise. Nous pouvons décrire le fait que chaque employé a un nom et un
salaire en définissant une nouvelle \emph{classe} :

\begin{lstlisting}[language=Java]
class Employee {
    String name;  // le nom de l'employe
    int salary;   // le salaire de l'employe
}
\end{lstlisting}

Les classes permettent de créer des \emph{objets}. Dans notre exemple, chaque
objet de la classe \texttt{Employee} représente un employé, ce qui facilite
...
Les deux objets que nous avons créés et placés dans les variables locales
\texttt{person1} et \texttt{person2} sont appelés des \emph{instances} de la
classe \texttt{Employee}. Les variables \texttt{name} et \texttt{salary} sont
des \emph{variables d'instance} : comme elles ne sont pas \texttt{static},
elles appartiennent aux instances, et chaque instance possède son propre
\texttt{name} et son propre \texttt{salary}.

\subsection{Initialiser les objets}



Dans l'exemple précédent, nous avons d'abord créé l'objet, puis défini les
...
Comme les variables de classe, les variables d'instance sont automatiquement
initialisées à \texttt{0} (pour les nombres), \texttt{false} (pour les booléens)
ou \texttt{null} (pour les autres types). C'est dangereux si l'on oublie
d'initialiser un champ :

\begin{lstlisting}[language=Java]
Employee person1 = new Employee();
person1.name = "Peter";
// oops, le salaire vaut 0
\end{lstlisting}

Il existe plusieurs manières d'éviter cela. Une solution consiste à
initialiser la variable dans la définition de la classe :
...
Bien sûr, cela n'est utile que si l'on veut que tous les employés commencent
avec un salaire de 10000. L'autre solution consiste à définir un
\emph{constructeur} dans votre classe. Le constructeur est une méthode spéciale
qui porte le même nom que la classe. Il peut avoir des paramètres mais n'a pas
de type de retour :
...
Si vous fournissez un constructeur pour votre classe, le compilateur Java
vérifiera que vous l'utilisez bien pour créer de nouveaux objets :

\begin{lstlisting}[language=Java]
Employee person1 = new Employee("Peter", 42000);
// OK. Nous avons maintenant un nouvel employe avec
// person1.name "Peter"
// person1.salary 42000

Employee person2 = new Employee();
// pas autorise. Il faut utiliser le constructeur !
\end{lstlisting}

Dans notre exemple, le constructeur prenait deux paramètres \texttt{n} et
\texttt{s} et les utilisait pour initialiser les variables d'instance
\texttt{name} et \texttt{salary} d'un nouvel objet \texttt{Employee}. Mais
comment le constructeur sait-il quel objet initialiser ? Devons-nous lui
indiquer que le nouvel objet est dans la variable \texttt{person1} ?
Heureusement, c'est plus simple : le constructeur peut toujours accéder à
l'objet en cours de construction via le mot-clé \texttt{this}. Ainsi, la ligne
\texttt{this.name = n;} signifie que la variable d'instance \texttt{name} du
nouvel objet est initialisée avec la valeur du paramètre \texttt{n}.

Nous pourrions même utiliser les mêmes noms pour les paramètres et les
...
Comme pour les variables de classe, il faut faire attention au masquage
(\emph{shadowing}). Sans \texttt{this.} devant le nom, le compilateur supposera
que vous désignez le paramètre. Erreur classique :
...
class Employee {
    ...
    Employee(String name, int salary) {
        name = name;     // oops, this.name n'est pas modifie
        salary = salary; // oops, this.salary n'est pas modifie
    }
}
\end{lstlisting}
...
Comme les variables de tableaux ou de type \texttt{String}, les variables
d'objets contiennent une \emph{référence} vers l'objet en mémoire. L'objet
lui-même contient les variables d'instance. Pour l'employé \texttt{Peter}, on
obtient la structure suivante :
...
\begin{minipage}[t]{0.45\textwidth}
\begin{lstlisting}
person1 -> Employee{name="Peter", salary=42000}
\end{lstlisting}
\end{minipage}
\end{tabular}
\end{center}

Pour cette raison, ce que nous avons déjà dit sur les tableaux et les chaînes
vaut aussi pour les objets : affecter une variable d'objet à une autre variable
ne copie que la référence. Comparer deux objets avec \texttt{==} compare leurs
références, pas le contenu :

\begin{lstlisting}[language=Java]
Employee person1 = new Employee("Peter", 42000);
Employee person2 = new Employee("Peter", 42000);
System.out.println(person1 == person2); // false. Deux objets distincts.

Employee person3 = person1;
System.out.println(person1 == person3); // true. Meme objet.
\end{lstlisting}
...
Beaucoup de choses que vous pouvez faire avec des types primitifs et des
chaînes, vous pouvez aussi les faire avec des objets. Par exemple, vous pouvez
créer des tableaux d'objets. Les éléments d'un nouveau tableau d'objets sont
automatiquement initialisés à \texttt{null}, comme dans cet exemple :

\begin{lstlisting}[language=Java]
Employee[] myTeam = new Employee[3];
myTeam[0] = new Employee("Peter", 42000);
myTeam[1] = new Employee("Anna", 45000);
System.out.println(myTeam[0].name); // "Peter"
System.out.println(myTeam[1].name); // "Anna"
System.out.println(myTeam[2].name); // Erreur ! myTeam[2] vaut null
\end{lstlisting}

Vous pouvez aussi avoir des variables de classe ou d'instance qui sont des
variables d'objets. Elles seront également initialisées à \texttt{null} si
vous ne fournissez pas de valeur initiale. Dans l'exemple suivant, nous avons
ajouté une nouvelle variable d'instance \texttt{boss} à notre \texttt{Employee} :
...
public class Main {
    public static void main(String[] args) {
        // Anna n'a pas de boss
        Employee anna = new Employee("Anna", 45000, null);
        // Anna est la boss de Peter
        Employee peter = new Employee("Peter", 42000, anna);
    }
}
\end{lstlisting}

Exercice : prenez une feuille de papier et dessinez le modèle mental pour
l'objet représentant Peter.

Question : dans l'exemple ci-dessus, quelle valeur donne-t-on à la variable
d'instance \texttt{boss} pour un employé qui n'a pas de supérieur ?

\subsection{Méthodes}



Dans l'exemple suivant, nous définissons une méthode statique
\texttt{increaseSalary()} pour augmenter le salaire d'un employé :
...
public class Main {
    static void increaseSalary(Employee employee, int raise) {
        // on augmente le salaire seulement si la hausse est < 10000
        if (raise < 10000) {
    ...
    public static void main(String[] args) {
        ...
        // Anna et Peter reçoivent une augmentation
...
Le code ci-dessus fonctionne. Mais en programmation orientée objet, on préfère
généralement que toutes les méthodes qui modifient les variables d'instance
d'un objet soient définies à l'intérieur de la classe. Dans un programme
important, cela facilite la compréhension. Pour cela, on remplace la méthode
statique \texttt{increaseSalary()} de \texttt{Main} par une méthode non
statique dans la classe \texttt{Employee} :
...
Comme \texttt{increaseSalary()} est maintenant une méthode non statique de
\texttt{Employee}, nous pouvons l'appeler directement sur un objet
\texttt{Employee}. Aucun paramètre \texttt{employee} n'est nécessaire car, à
l'intérieur de la méthode, \texttt{this} référence l'objet sur lequel la
méthode a été appelée. Nous écrivons donc simplement
\texttt{anna.increaseSalary(2000)} pour modifier le salaire d'Anna.

\subsection{Restreindre l’accès}



L'avantage de notre méthode \texttt{increaseSalary()} est de pouvoir imposer
une limite de 10000 euros :) Cependant, rien n'empêche un programmeur de
l'ignorer et de modifier le salaire à la main :
...
Ce genre d'erreur peut vite arriver dans un programme composé de centaines de
classes. On peut l'éviter en déclarant la variable d'instance \texttt{salary}
comme \texttt{private} :
...
Une variable d'instance \texttt{private} n'est accessible qu'à l'intérieur de
la classe. L'accès \texttt{anna.salary += 150000} dans \texttt{Main} ne
fonctionne donc plus. Mission accomplie...

Malheureusement, cela signifie aussi que l'on ne peut plus afficher le salaire
d'Anna via \texttt{System.out.println("New salary of Anna is " + anna.salary)}.
Pour corriger cela, on peut ajouter une méthode \texttt{getSalary()} dont le
seul but est de renvoyer la valeur de \texttt{salary}. Voici la nouvelle
version :
...
Supposons que nous écrivions un jeu (par exemple un RPG). Nous implémentons les
armes comme des objets de la classe \texttt{Weapon}. Les dégâts infligés par
une arme dépendent de son niveau, et son prix aussi. Le code pourrait ressembler
à ceci :
...
Avant de continuer, étudiez attentivement le programme ci-dessus et assurez-vous
de le comprendre. Exécutez-le dans IntelliJ. Les choses vont se compliquer !

Dans notre jeu, il existe aussi un type d'arme spécial, les \texttt{Mighty Sword}.
Ces épées infligent toujours 1000 points de dégâts, indépendamment de leur
niveau. En Java, on peut implémenter ce type d'arme ainsi :
...
La première ligne indique que la classe \texttt{MightySword} \emph{étend} la
classe \texttt{Weapon}. On dit que \texttt{MightySword} est une \emph{sous-classe}
(ou \emph{sous-type}) de \texttt{Weapon}, ou encore que \texttt{Weapon} est une
\emph{super-classe} de \texttt{MightySword}. En pratique, cela signifie que tout
ce que l'on peut faire avec des objets \texttt{Weapon}, on peut aussi le faire
avec des objets \texttt{MightySword} :
...
À première vue, il semble y avoir une erreur dans le \texttt{main()}. Pourquoi
la ligne \texttt{weapon = new MightySword(...)} n'est-elle pas une erreur de
type ? À gauche, la variable \texttt{weapon} est de type \texttt{Weapon}, et à
droite, on crée un objet \texttt{MightySword}. C'est accepté car Java applique
la règle suivante :

\textbf{Règle 1 :} Une variable de type \texttt{X} peut contenir une référence
vers un objet de la classe \texttt{X} ou vers un objet d'une sous-classe de
\texttt{X}.

Grâce à cette règle, le compilateur accepte de placer une référence vers un
\texttt{MightySword} dans une variable déclarée comme \texttt{Weapon}. Pour Java,
les instances de \texttt{MightySword} sont des \texttt{Weapon} particulières.

La ligne suivante semble également étrange :

\begin{lstlisting}[language=Java]
System.out.println("Price is " + weapon.getPrice());
\end{lstlisting}

Notre classe \texttt{MightySword} n'a pas défini de méthode \texttt{getPrice},
alors pourquoi \texttt{weapon.getPrice()} est-il possible ? C'est une autre
règle de Java :

\textbf{Règle 2 :} La sous-classe hérite des méthodes de sa super-classe. Les
méthodes définies dans une classe \texttt{X} peuvent être utilisées sur des
objets d'une sous-classe de \texttt{X}.
...
À partir de \texttt{Weapon weapon}, on pourrait s'attendre à ce que
\texttt{weapon.getSimpleDamage()} appelle la méthode de la classe \texttt{Weapon}.
Mais si vous regardez la sortie du programme, c'est la méthode
\texttt{getSimpleDamage()} de la classe \texttt{MightySword} qui est appelée.
Pourquoi ? Parce que \texttt{weapon} contient une référence vers un objet
\texttt{MightySword}. La règle est :

\textbf{Règle 3 :} Soit \texttt{x} une variable de type \texttt{X} (où \texttt{X}
est une classe) et assignons-lui un objet de classe \texttt{Y} (où \texttt{Y} est
une sous-classe de \texttt{X}). Lorsque vous appelez une méthode sur \texttt{x}
et que cette méthode est définie dans \texttt{X} et dans \texttt{Y}, la JVM
exécute la méthode définie dans \texttt{Y}.

Pour une instance de \texttt{MightySword}, appeler \texttt{getSimpleDamage()}
exécutera toujours la version définie dans cette classe. On dit que la méthode
\texttt{getSimpleDamage()} dans \texttt{MightySword} \emph{surcharge} la méthode
de \texttt{Weapon}. C'est pourquoi nous avons annoté la méthode avec
\texttt{@Override}.

Avec ces trois règles, pouvez-vous deviner ce qui se passe ici ?

\begin{lstlisting}[language=Java]
System.out.println("Double damage is " + weapon.getDoubleDamage());
\end{lstlisting}

Selon la règle 2, \texttt{MightySword} hérite de \texttt{getDoubleDamage()}.
Voyons comment cette méthode est définie dans \texttt{Weapon} :
...
Cette méthode appelle \texttt{this.getSimpleDamage()}. Quelle version de
\texttt{getSimpleDamage()} sera appelée ? Celle de \texttt{Weapon} ou celle de
\texttt{MightySword} ? Il faut se souvenir de la règle 3 : \texttt{this}
référence l'objet sur lequel la méthode est appelée. Comme notre objet est un
\texttt{MightySword}, la version de \texttt{MightySword} sera appelée. Le fait
que \texttt{getDoubleDamage} soit définie dans \texttt{Weapon} ne change rien.

\subsection{Super}



Il reste un détail dans notre classe \texttt{MightySword} : le constructeur :
...
Dans le constructeur, le mot-clé \texttt{super} désigne le constructeur de la
super-classe de \texttt{MightySword}, c'est-à-dire \texttt{Weapon}. La ligne
\texttt{super(name, level)} appelle simplement le constructeur défini dans
\texttt{Weapon}.

\texttt{super} peut aussi être utilisé dans les méthodes. Imaginons que nous
voulions définir une nouvelle arme \texttt{ExpensiveWeapon} qui coûte
exactement 100 de plus qu'une arme normale :
...
L'expression \texttt{super.getPrice()} appelle la méthode \texttt{getPrice()}
telle qu'elle est définie dans la super-classe \texttt{Weapon}. Cela signifie
que \texttt{super} permet d'appeler des méthodes de la super-classe, ce qui
serait autrement impossible pour des méthodes redéfinies à cause de la règle 3.

\subsection{L'annotation \texttt{@Override}}



L'annotation \texttt{@Override} n'est pas strictement nécessaire en Java (le
compilateur n'en a pas besoin), mais elle aide à éviter des erreurs. Par
exemple, imaginons que vous fassiez une faute d'orthographe en écrivant
\texttt{getSimpleDamage()} :
...
class MightySword extends Weapon {
    ...
    @Override
    int getSimpleDamag() {
        // oops, on a oublie le "e"
        return 1000;
    }
}
\end{lstlisting}

À cause de cette faute, le code n'écrase rien : il introduit une nouvelle
méthode \texttt{getSimpleDamag()}. Grâce à \texttt{@Override}, le compilateur
peut signaler l'erreur.

\subsection{Étendre, étendre\ldots{}}



Une sous-classe peut non seulement redéfinir des méthodes, mais aussi ajouter
de nouvelles variables d'instance et de nouvelles méthodes. Par exemple,
définissons une \texttt{MagicSword} qui inflige des dégâts magiques :
...
class MagicSword extends MightySword {
    ...
    MagicSword(String name, int level, int magicLevel) {
        super(name, level); // appel du constructeur de MightySword
...
Comme vous le voyez, on peut créer des sous-classes de sous-classes. Notez que
le constructeur utilise encore \texttt{super} pour appeler le constructeur de
la super-classe, puis initialise la nouvelle variable \texttt{magicLevel}.

Comment appeler \texttt{getMagicDamage()} ? Peut-on écrire :
...
La réponse est non. La règle 3 ne s'applique qu'aux méthodes définies à la fois
dans la sous-classe et dans la super-classe. Ce n'est pas le cas ici. Le
compilateur refusera \texttt{weapon.getMagicDamage()} car, en se basant sur
\texttt{Weapon weapon}, il ne peut pas savoir que l'objet référencé possède
\texttt{getMagicDamage()}. Cela peut sembler stupide, mais considérez :

\begin{lstlisting}[language=Java]
Weapon weapon = new MagicSword("Elven sword", 7, 3);
weapon = new Weapon("Dagger", 1);
System.out.println(weapon.getMagicDamage());
// ne compile pas, heureusement !
\end{lstlisting}

Pour appeler \texttt{getMagicDamage()}, il faut convaincre le compilateur que
la variable référence bien un \texttt{MagicSword}. Par exemple, en changeant le
type de la variable :
...
Ainsi, il est clair que la variable référence un \texttt{MagicSword} (ou une
sous-classe ; rappelez-vous la règle 1).

Alternativement, on peut faire un cast :
...
Mais soyez prudent avec les casts : le compilateur les accepte, mais une erreur
provoquera un problème à l'exécution :
...
Les trois règles permettent d'écrire du code et des structures de données qui
fonctionnent avec des objets de classes différentes. Par exemple, grâce à la
règle 1, on peut définir un tableau contenant différents types d'armes :
...
Et grâce aux règles 2 et 3, on peut écrire des méthodes qui fonctionnent pour
...
Bien que \texttt{getPriceOfInventory()} semble ne fonctionner qu'avec des
\texttt{Weapon}, elle fonctionne aussi avec toutes les sous-classes de
\texttt{Weapon}. C'est ce qu'on appelle le \emph{polymorphisme de sous-type}.
Si un \texttt{ExpensiveWeapon} est dans le tableau, la règle 3 garantit que
\texttt{weapon.getPrice()} appelle la méthode définie dans \texttt{ExpensiveWeapon}.

Conclusion : il existe une différence entre ce que le compilateur voit dans le
code source et ce qui se passe réellement à l'exécution. Le compilateur ne
vérifie l'existence d'une méthode que dans le type déclaré de la variable. Mais
pendant l'exécution, l'important est l'objet réellement référencé. On dit que
\emph{le typage du compilateur est statique}, tandis que \emph{les appels de
méthodes de la JVM sont dynamiques}.

\subsection{La hiérarchie de classes}



Si l'on prend toutes les classes d'armes définies précédemment, on obtient une
\og hiérarchie de classes \fg{} qui montre les relations entre sous-classes et
super-classes.

La classe \texttt{Object} située au-dessus de \texttt{Weapon} n'a pas été
définie par nous : elle est créée automatiquement par Java et est la
super-classe de \emph{tous} les types non primitifs, y compris les tableaux et
les chaînes ! Une variable de type \texttt{Object} peut donc référencer n'importe
quelle valeur non primitive :

\begin{lstlisting}[language=Java]
Object o;
o = "Hello";                 // OK
o = new int[]{1, 2, 3};       // OK aussi
o = new MagicSword("Elven sword", 7, 3); // toujours OK
\end{lstlisting}

La documentation de \texttt{Object} se trouve à l'adresse
\url{https://docs.oracle.com/javase/8/docs/api/java/lang/Object.html}. Cette
classe définit plusieurs méthodes intéressantes applicables à tous les objets.
L'une d'elles est \texttt{toString()}, appelée par des méthodes comme
\texttt{String.valueOf()} ou \texttt{System.out.println()} lorsqu'on leur passe
un objet. En redéfinissant cette méthode dans notre propre classe, on obtient
un affichage agréable :
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(peter); // appelle toString() de Player
    }
}
\end{lstlisting}

La méthode \texttt{toString()} est déclarée \texttt{public} dans \texttt{Object};
quand on la redéfinit, on doit donc la déclarer \texttt{public} aussi. Nous
reviendrons sur la signification de \texttt{public} plus tard.

Une autre méthode intéressante est \texttt{equals()}. Nous avons déjà appris
qu'il faut utiliser \texttt{equals()} pour comparer le contenu de deux chaînes,
car \texttt{==} compare seulement les références. Cela est recommandé aussi
pour nos objets. Mais comparer des objets est plus difficile que comparer des
chaînes. Pour notre classe \texttt{Player}, quand deux joueurs sont-ils égaux ?
Java ne peut pas répondre à cette question : nous devons fournir notre propre
implémentation de \texttt{equals()}. Par exemple, deux \texttt{Player} sont
égaux s'ils ont le même nom et la même année de naissance :
...
class Player {
    ...
    public boolean equals(Object obj) {
        if (this == obj) {
            return true; // meme objet !
        } else if (obj == null) {
            return false; // parametre null
        } else if (this.getClass() != obj.getClass()) {
            return false; // types differents
        } else {
            Player p = (Player) obj;
...
Que se passe-t-il ici ? Une difficulté avec \texttt{equals()} est qu'elle peut
être appelée avec un paramètre \texttt{null} ou un objet qui n'est pas une
instance de \texttt{Player}. Avant de comparer, il faut effectuer des tests,
dont celui du type :
...
Si tous ces tests passent, on peut enfin comparer le nom et l'année de
naissance. Notez qu'il existe d'autres difficultés avec \texttt{equals()}
liées à \texttt{hashCode()}, qu'il faut toujours redéfinir ensemble, comme
ci-dessus.
...
Utiliser \texttt{Object} est utile quand on veut écrire des méthodes qui
fonctionnent avec tous les types d'objets. Par exemple, un inconvénient des
tableaux par rapport aux listes Python est qu'ils ne peuvent pas changer de
taille. Dans le package \texttt{java.util}, la classe \texttt{ArrayList} permet
cela :
...
public class Main {
    public static void main(String[] args) {
        ...
        // ajoute deux elements a la fin de la liste
        list.add("Hello");
        list.add(new int[]{1, 2, 3});
        System.out.println(list.size()); // nombre d'elements
        System.out.println(list.get(0)); // premier element
    }
}
\end{lstlisting}

Comme on le voit, la méthode \texttt{add()} accepte n'importe quelle référence
(y compris vers des tableaux et des chaînes). De manière simplifiée, on peut
imaginer que \texttt{ArrayList} ressemble à ceci :

\begin{lstlisting}[language=Java]
public class ArrayList {
    // les elements ajoutes
    private Object[] elements;

    public void add(Object obj) {
        // ajoute "obj" dans le tableau
        // ...
    }

    public Object get(int index) {
        // renvoie l'objet en position "index"
        // ...
    }
}
\end{lstlisting}
...
Les boucles \texttt{for} fonctionnent aussi sur les \texttt{ArrayList} :
...
Malheureusement, les types primitifs ne sont pas des sous-classes de
\texttt{Object}. On ne peut donc pas ajouter directement un \texttt{int} à une
\texttt{ArrayList}, sans l'aide du compilateur :

\begin{lstlisting}[language=Java]
list.add(3); // est-ce que ca marche ?
\end{lstlisting}

Une solution consiste à écrire une classe dont l'unique but est de stocker la
valeur \texttt{int} dans un objet que l'on peut ajouter à la liste :
...
Ce procédé s'appelle \emph{boxing} car on met la valeur 3 dans une petite
\og boîte \fg{} (l'objet \texttt{IntObject}). Heureusement, on n'a pas besoin
d'écrire notre propre classe : \texttt{java.lang} fournit déjà \texttt{Integer} :

\begin{lstlisting}[language=Java]
// Integer est une classe du package java.lang
Integer value = Integer.valueOf(3);
list.add(value);
\end{lstlisting}

Le package \texttt{java.lang} contient aussi \texttt{Long}, \texttt{Float}, etc.
pour les autres types primitifs.

Le boxing est pénible et n'existe que parce que les types primitifs ne sont pas
des sous-classes de \texttt{Object}. Heureusement, le compilateur peut le faire
pour vous : c'est l'\emph{autoboxing}. Vous pouvez écrire :

\begin{lstlisting}[language=Java]
list.add(3); // appelle automatiquement Integer.valueOf(3)
\end{lstlisting}

L'autoboxing n'est pas limité à \texttt{ArrayList}. Il fonctionne partout où
vous assignez une valeur primitive à une variable d'un type classe compatible.
L'opération inverse, \emph{unboxing}, est aussi automatique :

\begin{lstlisting}[language=Java]
// autoboxing
// identique a : Integer value = Integer.valueOf(3);
Integer value = 3;

// auto-unboxing
// identique a : int i = value.intValue();
int i = value;
\end{lstlisting}

\subsection{ArrayList et génériques}



Le fait que \texttt{ArrayList} utilise \texttt{Object} a un gros inconvénient.
Comme \texttt{get()} retourne un \texttt{Object}, il faut faire un cast pour
retrouver le type original :
...
Même si nous savons que la liste contient uniquement des chaînes, le
compilateur exige le cast. C'est pénible et peut entraîner des erreurs qui
n'apparaissent qu'à l'exécution. Heureusement, Java propose les
\emph{génériques} :
...
La syntaxe \texttt{ArrayList<String>} indique au compilateur que \texttt{add()}
n'accepte que des \texttt{String}, et que \texttt{get()} retourne un
\texttt{String}. Le cast n'est plus nécessaire (il existe encore, mais le
compilateur l'ajoute automatiquement dans le \texttt{.class}).

Vous verrez d'autres exemples de génériques plus tard. Pour donner un aperçu,
voici la forme réelle de \texttt{ArrayList} :
...
Le \texttt{E} est un \emph{paramètre de type}. Il représente le type des
éléments stockés dans la liste. En créant notre liste avec
\texttt{ArrayList<String>}, on dit au compilateur que \texttt{E = String}. Les
méthodes \texttt{add()} et \texttt{get()} sont alors comprises comme
\texttt{void add(String obj)} et \texttt{String get(int index)}.

\subsection{Surcharge de méthodes avec des paramètres différents}



En Java, il est permis d'avoir deux méthodes portant le même nom à condition
qu'elles aient des paramètres différents. C'est la \emph{surcharge} de méthodes.
Exemple :
...
Si l'on appelle \texttt{set()}, le compilateur sait quelle version utiliser en
regardant les paramètres :

\begin{lstlisting}[language=Java]
Player player = new Player("Peter", 1993);
player.set("Pierre", 1993);
// appelle la version avec String et int
\end{lstlisting}

\subsection{Surcharge avec des paramètres de sous-classes}



Il faut être prudent quand on surcharge des méthodes dont les paramètres sont
des classes et des sous-classes. Voici un exemple minimal :
...
class Player {
    ...
    void giveWeapon(MightySword weapon) {
        this.weapon = weapon;
        this.power = 10; // une MightySword augmente la puissance
    }
}
...
Que va afficher \texttt{System.out.println(player.power)} après avoir donné une
\texttt{MightySword} ? Surprenant : cela affiche \texttt{0}. La méthode
\texttt{void giveWeapon(MightySword weapon)} n'est \emph{pas} appelée alors que
l'objet est bien une \texttt{MightySword}. L'explication : le compilateur ne
regarde que le type déclaré dans le code source. Ici, la variable \texttt{weapon}
est de type \texttt{Weapon}, donc c'est la version \texttt{giveWeapon(Weapon)}
qui est appelée. Il ne peut pas savoir qu'à l'exécution la variable contient un
\texttt{MightySword}.

Leçon : les appels de méthode sont dynamiques uniquement pour l'objet sur
lequel la méthode est appelée (règle 3), pas pour les arguments.

La bonne façon d'appeler \texttt{giveWeapon()} pour une \texttt{MightySword} est :
...
\subsection{Surcharge avec correspondance la plus proche}



Que se passe-t-il si l'on appelle une méthode surchargée mais qu'il n'existe
pas de version qui corresponde exactement au type de l'argument ? Voici le même
exemple, avec une troisième classe \texttt{MagicSword} qui est une sous-classe
de \texttt{MightySword} :
...
Quelle version de \texttt{giveWeapon()} sera appelée si l'argument est un
\texttt{MagicSword} ? Dans ce cas, le compilateur choisit la version dont le
type est le plus proche de \texttt{MagicSword}, ici
\texttt{void giveWeapon(MightySword weapon)}.

\subsection{Héritage multiple}



Si l'on reprend nos exemples avec \texttt{ExpensiveWeapon} et \texttt{MightySword},
on pourrait être tenté de créer une classe \texttt{ExpensiveMightySword} qui
hérite des deux :
...
// Pas autorise en Java !
// On ne peut pas etendre DEUX classes.
...
Quelle implémentation de \texttt{getPrice} doit être appelée ? Celle de
\texttt{ExpensiveWeapon} ou de \texttt{MightySword} ? Comme ce n'est pas clair,
l'héritage multiple est interdit en Java. D'autres langages l'autorisent sous
conditions ou avec des règles supplémentaires. Par exemple, en C\# la classe
\texttt{ExpensiveMightySword} devrait redéfinir \texttt{getPrice()}. En Python,
la méthode de \texttt{ExpensiveWeapon} serait appelée car cette classe apparaît
en premier dans la déclaration.

Si vous voulez en savoir plus, consultez
\url{https://en.wikipedia.org/wiki/Multiple_inheritance}.

Cependant, Java propose un autre concept, les \emph{interfaces}, qui peuvent
souvent servir de substitut à l'héritage multiple. Nous les verrons plus loin.

\subsection{Le mot-clé \texttt{final}}



Comme \texttt{private}, le mot-clé \texttt{final} ne change pas le comportement
de votre programme. Son rôle est d'empêcher certaines erreurs (nous verrons
d'autres usages de \texttt{final} plus tard). Sa signification dépend de
l'endroit où il est utilisé.

\subsection{Paramètres \texttt{final}}



Si vous déclarez un paramètre comme \texttt{final}, sa valeur ne peut pas être
changée dans la méthode. Cela évite des erreurs comme :

\begin{lstlisting}[language=Java]
// calcule la somme des nombres de 1 a n
int calculateSum(final int n) {
    // <--- avez-vous vu le "final" ?
    int sum = 0;
    for (int i = 1; i < n; i++) {
        n += i; // oops, je voulais ecrire sum += i
    }
    return sum;
}
\end{lstlisting}

Dans cet exemple, \texttt{n += i} sera refusé par le compilateur car \texttt{n}
est \texttt{final}.

Notez que si une variable contient une référence vers un tableau ou un objet,
la déclarer \texttt{final} n'empêche pas de modifier le contenu du tableau ou
de l'objet. Cela vaut pour les autres usages de \texttt{final}. Exemple :

\begin{lstlisting}[language=Java]
void increment(final int[] a) {
    a[0]++; // cela fonctionne toujours
}
\end{lstlisting}

\subsection{Variables locales \texttt{final}}



Les variables locales déclarées \texttt{final} ne peuvent pas changer de valeur
après avoir été initialisées. Le code suivant est refusé :
...
int calculateSumSquare(int n) {
    ...
    int sum = 0;
    for (int i = 1; i < n2; i++) {
        n2 += i; // oops, je voulais ecrire sum += i
    }
    return sum;
}
\end{lstlisting}

\subsection{Méthodes \texttt{final}}



Les méthodes déclarées \texttt{final} ne peuvent pas être redéfinies dans une
sous-classe. Cela peut être utile si vous pensez que la méthode contient du
code important qu'une sous-classe pourrait casser. Exemple :
...
class Employee extends Person {
    @Override
    String getFullName() {
        // pas autorise. La methode est "final" dans Person
        return "Wolverine";
    }
}
\end{lstlisting}

Cependant, il faut réfléchir avant de déclarer une méthode \texttt{final}, car
cela limite fortement la flexibilité des sous-classes.

\subsection{Classes \texttt{final}}



Les classes déclarées \texttt{final} ne peuvent pas être étendues. Par exemple,
\texttt{String} est \texttt{final} car tous les programmes Java dépendent de son
comportement précis décrit dans la documentation. Une sous-classe pourrait
causer de sérieux problèmes.

\subsection{Variables de classe \texttt{final}}



Comme les variables locales \texttt{final}, les variables de classe déclarées
\texttt{final} ne peuvent pas être modifiées après initialisation. Un cas
typique est la déclaration d'une constante :
...
La convention de nommage Java recommande d'écrire les constantes en majuscules.

\subsection{Variables d'instance \texttt{final}}



Les variables d'instance \texttt{final} ne peuvent pas être modifiées après
initialisation. En général, on les initialise dans le constructeur. Exemple :
...
public class Main {
    public static void main(String[] args) {
        ...
        person.socialSecurityNumber = "12";
        // erreur !
    }
}
\end{lstlisting}

Une raison importante de déclarer une variable d'instance \texttt{final} est
quand elle fait partie de l'\og identité \fg{} d'un objet : quelque chose qui
ne doit jamais changer après la création de l'objet.

Notez qu'il est possible d'obtenir une variable non modifiable sans
\texttt{final}. Par exemple, on pourrait écrire :
...
Dans tous nos petits exemples, nous avons mis toutes les classes dans un seul
fichier \texttt{.java}. Ce n'est pas pratique dans des projets plus grands. La
règle générale (ou recommandation) est de placer chaque classe dans un fichier
séparé portant le même nom que la classe.

Java permet aussi de regrouper les classes dans des \emph{packages} en écrivant
une instruction \texttt{package} en première ligne du fichier :
...
Si vous mettez vos classes dans des packages, le compilateur s'attend à ce que
vous organisiez les fichiers source dans une arborescence de répertoires qui
correspond aux noms de packages. Dans notre exemple, les fichiers \texttt{.java}
doivent être placés dans un dossier \texttt{week3} à l'intérieur d'un dossier
\texttt{lepl1402} dans \texttt{src}.

Si vous ne mettez pas d'instruction \texttt{package}, le compilateur place vos
classes dans le \emph{package sans nom}. Dans ce cas, vous pouvez placer tous
vos fichiers directement dans \texttt{src}.

\subsection{Utiliser plusieurs packages}



En Java, les packages sont indépendants. Les classes d'un même package peuvent
être utilisées ensemble, comme dans l'exemple précédent.

Cependant, les classes de packages différents ne se \og voient \fg{} pas par
défaut. Par exemple, si l'on place \texttt{Person} dans le package
\texttt{lepl1402.week3.example} et \texttt{Main} dans \texttt{lepl1402.week3},
...
Dans cet exemple, nous avons fait trois modifications :

\begin{enumerate}
  \item Nous avons déclaré la classe \texttt{Person} comme \texttt{public}.
        Seules les classes publiques peuvent être utilisées par d'autres
        packages.
  \item Nous avons déclaré le constructeur de \texttt{Person} comme \texttt{public}.
        Encore une fois, seules les méthodes publiques sont accessibles.
  \item Nous avons ajouté un \texttt{import} dans \texttt{Main.java}. Cet
        \texttt{import} indique où se trouve la classe \texttt{Person}. Le nom
        \texttt{lepl1402.week3.example.Person} est le \emph{nom pleinement
        qualifié} de la classe.
\end{enumerate}

Comme alternative à \texttt{import}, on peut utiliser le nom pleinement
qualifié directement dans le code, mais cela le rend moins lisible :
...
\subsection{Pourquoi les packages sont-ils utiles ?}



Les packages ont deux avantages. D'abord, avec le mot-clé \texttt{public}, vous
contrôlez pour chaque classe et méthode si elle peut être utilisée par d'autres
packages. Par exemple, \texttt{java.lang} contient des classes comme
\texttt{String} ou \texttt{Integer}, déclarées \texttt{public}. Mais il contient
aussi des classes comme \texttt{CharacterData0E}, utilisées en interne et donc
non publiques.

Le second avantage est qu'ils fournissent des \emph{espaces de noms} séparés.
Un package \texttt{X} et un package \texttt{Y} peuvent contenir chacun une
classe \texttt{ABC}. En utilisant les noms pleinement qualifiés (ou \texttt{import}),
on indique sans ambiguïté si l'on veut \texttt{X.ABC} ou \texttt{Y.ABC}.

\subsection{Contrôle d'accès}



Récapitulons la visibilité des classes dans les packages :

\begin{enumerate}
  \item Les classes déclarées \texttt{public} sont visibles dans tous les packages.
  \item Les classes non publiques sont visibles uniquement dans leur package.
\end{enumerate}

Pour les membres de classe (méthodes, variables de classe, variables
d'instance), les règles sont plus complexes :

\begin{enumerate}
  \item Les membres \texttt{public} sont accessibles depuis tous les packages.
  \item Les membres \texttt{private} ne sont accessibles que dans leur classe.
  \item Les membres \texttt{protected} sont accessibles dans leur classe et dans
        les sous-classes.
  \item Les membres sans modificateur spécial sont accessibles dans la classe
        et dans le même package.
\end{enumerate}

\section{Classes abstraites}
\label{sec:abstract-classes}

Une classe abstraite en Java est une classe qui ne peut pas être instanciée
directement et qui est destinée à servir de classe parente. Les classes
abstraites sont utiles lorsque vous voulez fournir une base commune à plusieurs
sous-classes sans vouloir instancier cette base. Elles peuvent contenir des
méthodes concrètes et des méthodes abstraites (sans corps).

Imaginons un programme de dessin géométrique avec des calculs scientifiques,
comme le calcul de l'aire de différentes formes. La formule de l'aire dépend
de la forme, mais certaines fonctionnalités sont communes (par exemple,
afficher des informations). Le programme doit aussi permettre aux utilisateurs
d'ajouter de nouvelles formes.

Notre objectif est de respecter le \emph{principe Open/Closed} (OCP) : un module
doit être ouvert à l'extension mais fermé à la modification. Les classes
abstraites sont très utiles dans ce contexte.

\begin{lstlisting}[language=Java]
public abstract class Shape {
    protected String shapeName;
    // variable d'instance pour le nom de la forme

    public Shape(String name) {
        this.shapeName = name;
    }

    // Methode abstraite pour calculer l'aire
    public abstract double calculateArea();

    // Methode concrete definie dans la classe abstraite
...
Avec cette conception, ajouter de nouvelles formes est simple et ne nécessite
pas de modifier le code existant. Il suffit de créer une nouvelle sous-classe :
...
Pour calculer l'aire totale d'un tableau de formes, on peut créer une méthode
statique qui itère et appelle \texttt{calculateArea()} sur chaque forme :

\begin{lstlisting}[language=Java]
class ShapeUtils {
    // Methode statique pour calculer l'aire totale
...
Cette méthode reste valable même si vous ajoutez de nouvelles formes plus tard.

\section{Interfaces}
\label{sec:interfaces}

Une interface en Java est une classe entièrement abstraite : aucune de ses
méthodes n'a d'implémentation concrète. Les interfaces regroupent des méthodes
vides qui spécifient ce qu'une classe doit faire, pas comment elle le fait.

Un avantage des interfaces par rapport aux classes abstraites est qu'une classe
peut en implémenter plusieurs. Java ne permet pas d'étendre plusieurs classes.
Les interfaces offrent donc une flexibilité supérieure, même si elles ne
permettent pas autant de factorisation du code.
...
Considérons la classe \texttt{Book} suivante :
...
public class Book {
    ...
    // ... getters, setters, et autres methodes ...
}
\end{lstlisting}

Nous voulons trier une collection de livres par titre. On peut le faire en
implémentant l'interface \texttt{Comparable}, qui impose de définir
\texttt{compareTo()}. Cette méthode s'appuie sur \texttt{String.compareTo()}.
...
public class Book implements Comparable<Book> {
    ...
    public static void main(String[] args) {
        ...
        // tri par titre grace a Comparable
        for (Book book : books) {
...
Imaginons que ces livres soient affichés sur un site web. On veut pouvoir
trier non seulement par titre, mais aussi par auteur ou par année. Problème :
\texttt{Comparable} impose un seul \og ordre naturel \fg{}, ce qui rend difficile
l'ajout de critères.

Un principe important en conception orientée objet est le \emph{principe
Open/Closed} (OCP) : un module doit être ouvert à l'extension mais fermé à la
modification. Le \emph{pattern de délégation} permet d'améliorer le design.
Avec \texttt{Comparator}, l'algorithme de tri délègue la comparaison à un
objet, ce qui permet d'ajouter des critères sans modifier la classe \texttt{Book}
ni l'algorithme de tri.

Voici trois comparateurs, un par critère :
...
On peut alors trier par titre, auteur ou année en fournissant le comparateur :
...
Vous développez un système de gestion de documents. La classe \texttt{Document}
contient le contenu et vous voulez offrir une capacité d'impression en
déléguant cette responsabilité, pour respecter le principe de responsabilité
unique. Complétez le code :
...
En informatique, il est conseillé d'avoir un couplage faible entre objets (le
contraire est souvent appelé \og spaghetti code \fg{}). Le \emph{pattern Observer}
permet d'obtenir un couplage faible. Nous montrons d'abord son usage en GUI,
puis son implémentation.

\subsection{Observer sur des composants GUI}



En Java, les packages \texttt{swing} et \texttt{awt} facilitent la création
d'interfaces graphiques. Swing utilise un système basé sur l'observer pour
gérer des événements comme les clics. Dans l'exemple suivant, un bouton affiche
\og Thank you \fg{} lorsqu'on clique :
...
\texttt{ActionListener} est une interface contenant une méthode
\texttt{actionPerformed()}. Ici, \texttt{ButtonActionListener} l'implémente et
affiche un dialogue. Cependant, rien ne se passe tant que l'on n'associe pas un
listener au bouton via \texttt{addActionListener()}. Ainsi, à chaque clic,
\texttt{actionPerformed()} est appelée.

Il est important de comprendre le contrat : la méthode du listener est appelée
quand le bouton est pressé. On parle parfois d'\og attacher un callback \fg{} ou
de \og enregistrer un gestionnaire d'événements \fg{}. Ce principe est souvent
résumé par la phrase : \og Don't call us, we will call you. \fg{}

On peut enregistrer plusieurs listeners sur un même bouton. Par exemple, un
second listener pourrait compter le nombre de clics. Le bouton \texttt{JButton}
illustre ainsi l'observer vu du côté utilisateur. Voyons maintenant comment le
mettre en œuvre pour nos propres classes.

\subsection{Implémenter le pattern Observer}



Imaginons un compte bancaire sur lequel plusieurs personnes déposent de l'argent.
Chaque membre de la famille souhaite être alerté à chaque dépôt. Pour simplifier,
les notifications sont affichées dans la console. Voici le code complet :
...
public class ObservableAccount {
    ...
    public static void main(String[] args) {
        ...
        account.deposit(100);
        // affiche 4X "The account has changed. New value: 100"
        account.deposit(50);
        // affiche 4X "The account has changed. New value: 150"
    }
}
\end{lstlisting}

Dans ce contexte, le compte bancaire est le sujet observé (la classe
\texttt{ObservableAccount}). Cette classe maintient un solde, modifié par
\texttt{deposit()}. Les observateurs sont enregistrés dans une liste
(\texttt{LinkedList}) qui permet des ajouts rapides et des itérateurs.

À chaque dépôt, la valeur est mise à jour et tous les observateurs sont
notifiés via \texttt{accountHasChanged()}. Nous ne vérifions pas les doublons
dans la liste : c'est à l'utilisateur d'assurer l'unicité.

Notez que l'ordre de notification dépend de l'ordre d'enregistrement, car
nous utilisons une liste. Mais l'utilisateur ne doit pas faire l'hypothèse que
cet ordre est garanti : une autre collection (par exemple un ensemble) pourrait
changer l'ordre.

\paragraph{Exercice}

Dans cet exercice, vous utiliserez le pattern Observer avec Java Swing.
L'application \texttt{MessageApp} fournit une interface graphique simple :
les utilisateurs tapent un message et le soumettent. Ce message passe par un
\texttt{SpellChecker} puis doit être affiché par des observateurs.

Votre tâche est de faire fonctionner le tout : quand un message est soumis, il
est corrigé par le \texttt{SpellChecker}, puis ajouté à la zone de texte
(\texttt{textArea.append(String text)}).

Il est impératif que votre conception permette de remplacer facilement le
\texttt{SpellChecker} sans modifier \texttt{MessageApp}. De plus, la classe
\texttt{MessageSubject} doit rester découplée de \texttt{MessageApp} : elle ne
doit pas en dépendre ni même savoir qu'elle existe. Utilisez les interfaces
quand c'est possible. Avec les progrès du deep learning, nous prévoyons de
remplacer \texttt{StupidSpellChecker} par une version plus avancée : faites en
sorte que ce changement soit simple, sans modifier vos classes.
...
En pratique, on ne teste qu'un petit nombre de valeurs d'entrée, en espérant que
le logiciel fonctionne aussi pour les autres. Par exemple, si l'appel
\texttt{division(6, 3)} donne le bon résultat, on suppose que
\texttt{division(12, 6)} est aussi correct. À l'inverse, si le résultat pour
6 et 3 est incorrect, on sait qu'il y a un bug à corriger.

Cela signifie-t-il qu'il faut choisir des nombres au hasard ? Non, on peut
faire mieux :
...
L’état de la pile chaînée après avoir empilé 1, 5 et 3 dans cet ordre est illustré dans la figure suivante.
...
Une utilisation typique des piles est l’évaluation d’expressions arithmétiques, comme dans l’algorithme suivant :
...
La complexité temporelle de l’algorithme est clairement \(\mathcal{O}(n)\), où \(n\) est la taille de la chaîne d’entrée :
chaque jeton (qu’il s’agisse d’un nombre, d’un opérateur ou d’une parenthèse) est lu et traité exactement une fois.

\begin{itemize}
  \item Empiler et dépiler des éléments d’une pile prend un temps constant, \(\mathcal{O}(1)\).
  \item Les opérations arithmétiques (addition, soustraction, multiplication et division) sont effectuées en temps
        constant, \(\mathcal{O}(1)\).
\end{itemize}

Pour comprendre et se convaincre de la correction de l’algorithme, on peut chercher un invariant. On peut voir qu’une
expression entièrement parenthésée peut être représentée comme un arbre binaire sans parenthèses : les noeuds internes
sont les opérateurs et les feuilles sont les valeurs. L’algorithme utilise deux piles. Une pile (\texttt{ops}) est
dédiée aux opérateurs et l’autre (\texttt{vals}) aux valeurs (réduites). Le programme découpe la chaîne d’entrée
\texttt{args[0]} par espaces afin de traiter chaque jeton individuellement. Nous ne formalisons pas complètement
l’invariant ici, mais en donnons l’intuition. À tout instant pendant le traitement de l’expression :

\begin{enumerate}
  \item la pile \texttt{vals} contient les résultats de toutes les sous-expressions entièrement évaluées (sous-arbres
        réduits) rencontrées jusqu’ici ;
  \item la pile \texttt{ops} contient les opérateurs qui attendent leur opérande droit pour former une sous-expression
        (sous-arbre) complète pouvant être réduite ;
  \item pour chaque opérateur dans \texttt{ops}, son opérande gauche correspondant est déjà dans \texttt{vals}, en
        attente que son sous-arbre soit complété et réduit.
\end{enumerate}

La figure montre l’état des piles à trois étapes distinctes de notre exemple succinct.

Lorsqu’on rencontre un opérande, c’est comme rencontrer une feuille de cet arbre, et sa valeur est connue
immédiatement ; elle est donc empilée dans \texttt{vals}. Lorsqu’on rencontre un opérateur, il est empilé dans
\texttt{ops}. Cet opérateur attend son opérande droit pour former un sous-arbre complet ; son opérande gauche est déjà
sur \texttt{vals}. Lorsqu’une parenthèse fermante \texttt{")"} est rencontrée, cela indique la fin d’une sous-expression
entièrement parenthésée, correspondant à un sous-arbre entier de l’expression. Ce sous-arbre est « réduit » ou
« évalué » de la manière suivante :

\begin{enumerate}
  \item l’opérateur correspondant à ce sous-arbre est dépilé de \texttt{ops} ;
  \item l’opérande droit (valeur du sous-arbre droit) est dépilé de \texttt{vals} ;
  \item l’opérande gauche (valeur du sous-arbre gauche) est dépilé de \texttt{vals} ;
  \item l’opérateur est appliqué aux deux opérandes et le résultat (valeur du sous-arbre entier) est repoussé dans
        \texttt{vals}.
\end{enumerate}

Cet invariant capture l’essence de l’approche de l’algorithme : il parcourt l’arbre d’expression d’une sorte de manière
en profondeur, évaluant chaque sous-arbre dès qu’il est complètement identifié par sa parenthèse fermante. Cet
algorithme qui prend une \texttt{String} en entrée est un exemple d’interpréteur. Les langages de programmation
interprétés (comme Python) font de même mais acceptent des constructions légèrement plus complexes que les expressions
arithmétiques parenthésées.

\paragraph{Exercice.} Écrire un algorithme récursif d’évaluation d’expressions arithmétiques. Ce programme n’utilisera
pas de piles explicites, mais s’appuiera sur la pile d’appels.
